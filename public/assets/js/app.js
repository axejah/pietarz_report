...
async function getData() {
  const response = await fetch(`http://localhost:5005/api/csv/getReportData/${collectionId}`)
...
getData().then(data => {
  for (let i = 0; i < data.unsortedData.keywordLabel.length; i++) {
    ...
    let color = data.unsortedData.rankChange[i] < 0 ? 'style="color: red;"' : 'style="color: green;"';
  ...
  leastVolumeData.push(data.sortedData.byVolume.volumeData.slice(data.sortedData.byVolume.volumeData.length - 10, data.sortedData.byVolume.volumeData.length));
  leastVolumeLabel.push(data.sortedData.byVolume.volumeLabel.slice(data.sortedData.byVolume.volumeLabel.length - 10, data.sortedData.byVolume.volumeLabel.length));
  topMostDifficultData.push(data.sortedData.byDifficulty.difficultyData.slice(0, 10))
  topMostDifficultLabel.push(data.sortedData.byDifficulty.difficultyLabel.slice(0, 10))
